...
from the Excel workbook in `scripts/catalog-data/source`.

## Script

- `scripts/catalog-data/generate_catalog_json.py`
...
Default (uses first `.xlsx` found in `scripts/catalog-data/source`):
...
```bash
python scripts/catalog-data/generate_catalog_json.py \
  --products-out "public/data/products-grouped.json" \
  --additional-out "public/data/additional-images.json"
```

Optional sheet selection:

```bash
python scripts/catalog-data/generate_catalog_json.py --sheet "Sheet1"
```
...
- If no `--xlsx` is provided and no `.xlsx` exists in `scripts/catalog-data/source`, script fails with `FileNotFoundError`.
