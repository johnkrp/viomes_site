...
const tableStart = headerLineIndex + 2; // skip separator line after header
  ...
  while (i < lines.length) {
    ...
    mapping[code] = { analy, analy_en, tech, tech_en, care };
  ...
  for (const size of product.sizes || []) {
    for (const variant of size.variants || []) {
      ...
      if (mapping[code]) {
        ...
        // optional: set other fields if desired
        // if (mapping[code].tech) variant.specs_note = mapping[code].tech;
        // if (mapping[code].care) variant.care_note = mapping[code].care;
      }
    }
  }
}
