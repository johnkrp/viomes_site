...
const parseVariantSpecs = (description: string) => {
  ...
  const dimensionsMatch = text.match(
    /([dDØΦ]?\s*[\d.,]+\s*x\s*[hH]?[\d.,]+(?:\s*x\s*[hH]?[\d.,]+)?)/i,
  );
...
const ProductDetail = () => {
  ...
  const [leftPackshotAspect, setLeftPackshotAspect] = useState<
    string | undefined
  >(undefined);
  const [rightCarouselAspect, setRightCarouselAspect] = useState<
    string | undefined
  >(undefined);
  ...
  const safeSizeIndex = Math.min(
    selectedSizeIndex,
    Math.max(product.sizes.length - 1, 0),
  );
  ...
  const selectedVariant =
    selectedSize?.variants?.[safeColorIndex] || selectedSize?.variants?.[0];
  ...
  const rightCarouselImages = useMemo(() => {
    const bySelectedCode = selectedVariant?.code
    ...
    product.sizes.forEach((size) => {
      size.variants.forEach((variant) => {
        ...
        entries
          ...
          .forEach((url) => familyFallback.add(url));
      });
    });
  ...
  useEffect(() => {
    setLeftPackshotAspect(undefined);
  }, [selectedVariant?.image_url, product.representative_image]);

  useEffect(() => {
    setRightCarouselAspect(undefined);
  }, [
    carouselIndex,
    rightCarouselImages,
      ...
      <div className="mx-auto w-full max-w-none px-2 md:px-4 xl:px-6">
        ...
        <section className="mx-auto w-full grid grid-cols-1 xl:grid-cols-[260px_minmax(520px,1fr)_460px] gap-6 xl:gap-8 items-start">
          <div className="hidden xl:block pt-8">
            <div
              className="bg-background/45 p-5 xl:p-6"
              style={
                leftPackshotAspect
                  ? { aspectRatio: leftPackshotAspect }
                  : undefined
              }
            >
              <img
          ...
          <div className="pt-8 xl:pt-8">
            <div className="flex items-end justify-between gap-6">
              <div className="pb-1">
                <h1 className="text-[44px] leading-none font-semibold text-foreground/95">
                  {product.title}
                </h1>
                <p className="mt-2 text-[20px] leading-tight text-foreground/85">
            ...
            <div className="mt-5 bg-[hsl(var(--viomes-bg))] px-7 xl:px-8 py-4 xl:py-5">
            ...
            {selectedVariant?.excel_ar ? (
              <p className="mt-2 text-[18px] leading-tight text-foreground/70">
                {selectedVariant.excel_ar}
              </p>
            ) : null}
          </div>
          ...
          <div className="pt-4 xl:pt-0">
            <div className="relative">
              <button
                ...
                style={
                  rightCarouselAspect
                    ? { aspectRatio: rightCarouselAspect }
                    : undefined
                }
              >
                <img
                  ...
                  onLoad={(e) => {
                    const img = e.currentTarget as HTMLImageElement;
                    if (img?.naturalWidth && img?.naturalHeight) {
                      setRightCarouselAspect(
                        `${img.naturalWidth}/${img.naturalHeight}`,
                      );
                    }
                  }}
                  className="w-full h-full object-contain transition-all duration-1000 ease-in-out hover:scale-105 animate-fade-in"
