...
const colorMap: Record<string, ColorMeta> = {
  "ΣΚΟΎΡΟ ΠΡΆΣΙΝΟ ΕΛΙΆΣ": {
    hex: "#5E6738",
    english: "Dark Olive",
    greek: "Σκούρο Πράσινο Ελιάς",
    code: "143",
  },
  "ΔΙΆΦΑΝΟ ΠΟΡΤΟΚΑΛΊ": {
    hex: "#FF8000",
    english: "Clear Orange",
    greek: "Διάφανο Πορτοκαλί",
    code: "46",
  },
  "ΔΙΑΦΑΝΟ ΠΟΡΤΟΚΑΛΙ": {
    hex: "#FF8000",
  ...
  "ΣΚΟΎΡΟ ΚΥΠΑΡΙΣΣΊ": {
    hex: "#205C40",
    english: "Dark Cypress",
    greek: "Σκούρο Κυπαρισσί",
    code: "55",
  },
  "ΔΙΆΦΑΝΟ ΚΊΤΡΙΝΟ": {
    hex: "#FFE600",
    english: "Clear Yellow",
    greek: "Διάφανο Κίτρινο",
    code: "45",
  },
  "ΔΙΆΦΑΝΟ ΠΡΆΣΙΝΟ": {
    hex: "#B7DD79",
    english: "Clear Green",
    greek: "Διάφανο Πράσινο",
    code: "47",
  },
  "ΔΙΑΦΑΝΟ ΚΙΤΡΙΝΟ": {
    hex: "#FFE600",
  ...
  "ΓΑΛΆΖΙΟ ΣΚΟΎΡΟ": {
    hex: "#407EC9",
    english: "Deep Azure",
    greek: "Γαλάζιο Σκούρο",
    code: "24",
  },
  ΚΙΤΡΙΝΟΠΡΆΣΙΝΟ: {
    hex: "#A8AD00",
    english: "Chartreuse",
    greek: "Κιτρινοπράσινο",
    code: "12",
  },
  "ΠΡΆΣΙΝΟ ΣΚΟΎΡΟ": {
    hex: "#009A44",
    english: "Dark Green",
    greek: "Πράσινο Σκούρο",
    code: "36",
  },
  "ΔΙΑΦΑΝΟ ΛΕΥΚΟ": {
    english: "Clear White",
  ...
  "MΑΡΜΑΡΊ ΤΡΊΧΑ": {
    english: "Marble White",
    greek: "Mαρμαρί Τρίχα",
    code: "90",
  },
  "TITANIUM GREY": {
    hex: "#5B6770",
    english: "Titanium Grey",
    greek: "Γκρι Τιτανίου",
    code: "58",
  },
  "ΓΚΡΙ ΤΙΤΑΝΊΟΥ": {
...
export const matchesColorSelection = (
  variantColor: string,
  ...
  const variantTokens =
    splitMixedColorTokens(variantColor).map(normalizeColorKey);
  const selectedTokens =
    splitMixedColorTokens(selectedColor).map(normalizeColorKey);

  if (variantTokens.includes(selectedNorm)) return true;
  if (
    selectedTokens.length > 0 &&
    selectedTokens.every((token) => variantTokens.includes(token))
  ) {
    return true;
  }
...
export const resolveColor = (raw: string) =>
  resolveSingleMeta(raw).hex || hashColor(raw);

export const resolveColorTitle = (
  raw: string,
  language: "el" | "en" = "en",
) => {
...
export const resolveSwatchBackground = (raw: string) => {
  ...
  const colors = tokens.map(
    (token) => resolveSingleMeta(token).hex || hashColor(token),
  );
  if (colors.length === 2) {
